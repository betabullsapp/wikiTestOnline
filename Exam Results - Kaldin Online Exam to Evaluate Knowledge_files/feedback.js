...
window.Feedback = function( options ) {
    ...
    returnMethods = {
        ...
        send: function( adapter ) {
            ...
            var p = 0;
            for (var i = 0, len = options.pages.length, data = [], tmp; i < len; i++) {
                if ( (tmp = options.pages[ i ].data()) !== false ) {
                    data[ p++ ] = tmp;
                }
            }
            var newElm = { BrowserName : $.browser.name };
            data[ p++ ] = newElm;
            var newElm = { BrowserVersion : $.browser.version };
            data[ p++ ] = newElm;
            var newElm = { OS : $.os.name };
            data[ p++ ] = newElm;
